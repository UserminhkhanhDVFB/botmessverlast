...
module.exports.handleReply = async function({
  ...
  //Thêm, su?a ca´c câu tra? lo`i o? dây
	var D = [
    "Yêu em <3", "Hi, cha`o con vo? be´:3", "Vo? go?i co´ viê?c gi` không?", "D?, có em dây, yêu em không mà g?i <3. hmm...", `${m} su? du?ng callad dê? liên la?c vo´i admin!`, `${m} go?i em co´ viê?c gi` thê´`, `${m}, yêu em ko ma` go?i??`, `${m}, tôi yêu ba?n vl ?`, `${m}, ba?n co´ yêu tôi không ?`, `${m}, da? co´ em dây:3`, `${m}, yêu Admin bot di rô`i ha~y go?i`, `${m}, yêu em ?`, `${m}, [Go´c Donate] Ba?n co´ thê? donate cho tôi chu´?`, `${m}, Tao dây`
    ],
		y = D[Math.floor(Math.random() * D.length)];

  //Ca´c câu go?i bot
	["bot", "bot oi", "bot oi", "yêu bot", "bot dâu"].forEach((e => {
      
		let o = e[0].toUpperCase() + e.slice(1);
		if (i === e.toUpperCase() | i === e | o === i) {
			let o = h.threadName;
			modules = "Go?i bot:", console.log(modules, e + "|", o), a.sendMessage(y, s, (() => {
				var n = c;
				for (var n of c) a.sendMessage(`=== Bot Notification ===\n\n??Box Name: ${o}\n??ID box: ${l}\n??Name User: ${m} \n??ID User: ${g}\n??Time: ${d}\n??Go?i bot: ${e}`, n, ((e, a) => global.client.handleReply.push({
					name: this.config.name,
					author: r,
					messageID: a.messageID,
					messID: t,
	...
	event: e,
	api: a
}) => a.sendMessage("( \\_/)                                                                            ( •_•)                                                                            // >                                                            Ðua na~o cho ba?n la´p va`o dâ`u ne`.\nCo´ biê´t la` lê?nh Noprefix hay không?", e.threadID);