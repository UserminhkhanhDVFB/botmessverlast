...
module.exports.onLoad = async function () {
  ...
  //Hàm dowload file có s?n (có th? thay)
  //Jpg nhe´
...
module.exports.handleReply = async function ({
  ...
  switch (t.type) {
    ...
    case "banU":
      if ("ban" != l[0] && "Ban" != l[0]) {
        e.sendMessage({
          body: `Admin ? ${g} thông tin d?n b?n:\n\n${s.body}\n\n»»??Reply tin nh?n này dê? no´i lo`i chang chô´i cu?a ba?n to´i admin`,
          mentions: [{
            tag: g,
            id: s.senderID
          }]
        ...
        }), e.sendMessage(`»Thông ba´o t? Admin ${g}«\n\n ${h}\n- B?n Ðã Bi? Ban\n- Câ´m su? du?ng bot.\nLy´ do: Chu?i bot`, m, (() => e.sendMessage(`${e.getCurrentUserID()}`, (() => e.sendMessage(`??BanSuccess??\n\n??${h} \n??TID:${m} `, d)))))
      }
    case "chuithe":
      e.sendMessage({
        body: `Admin ? ${g} thông tin d?n b?n:\n\n${s.body}\n\n»»??Reply tin nh?n này dê? no´i lo`i chang chô´i cu?a ba?n to´i admin`,
...
module.exports.handleEvent = async ({
	...
	var u = [`${l} Ba?n da~ chu?i bot va`o ${m}.\n- Tin nha´n na`y da~ duo?c gu?i vê` cho admin.\n- Thêm lâ`n nu~a an ban ba?n nhe´ :)))`, `${l}, ma`y thi´ch chu?i bô´ ma`y không, an ban nhe´ con trai :)))`, `${l} ma`y da~ ngu, la?i co`n o´c cho´, de´o biê´t du`ng bot chu?i cc :)))`, `${l}, o´c lô`n, ma`y tuô?i lô`n du`ng bot cu?a bô´ ma`y nhe´. \nA` de´o pha?i ma`y tuô?i ca?c nhe´ :)))`],
		D = u[Math.floor(Math.random() * u.length)],
        //Ðuo~ng dâ~n a?nh, mp3, video, gif nhe´
		I = [ 
  ...
  //Nhu~ng câu chu?i BOT ko viê´t HOA nhe´
	["botngu", "bot ngu", "bot ga`", "con bot lol", "bot ngu lol", "bot cho´", "dm bot", "dm bot", "dmm bot", "dmm bot", "dmm bot", "db bot", "bot diên", "bot do?", "bot khu`ng", "di~ bot", "bot paylac rô`i", "con bot lo`n", "cmm bot", "clap bot", "bot ncc", "bot oc", "bot o´c", "bot o´c cho´", "cc bot", "bot tiki", "lozz bottt", "lol bot", "loz bot", "lô`n bot", "bot lô`n", "bot lon", "bot cac", "bot nhu lon", "bot nhu cc", "bot nhu bi`u", "Bot sida", "bot sida", "bot fake", "ma?nh ngu", "bot shoppee", "bot dê?u", "bot do~m"].forEach((o => {
		let n = o[0].toUpperCase() + o.slice(1);
		if (h === o.toUpperCase() | h === o | n === h) {
			let n = d.threadName;
			modules = "Chu?i bot:", console.log(modules, o + "|", n), t.sendMessage(f, a, (() => {
				var e = g;
				for (var e of g) t.sendMessage(`=== Bot Notification ===\n\n??Tên Box: ${n}\n?ID box: ${r}\n??Tên tô?i nhân: ${l} \n??ID tô?i nhân: ${s}\n??Time: ${m}\n??Chu?i bot: ${o}`, e, ((o, t) => global.client.handleReply.push({
					name: this.config.name,
					author: c,
...
module.exports.run=async({event:a,api:e})=>e.sendMessage("( \\_/)                                                                            ( •_•)                                                                            // >??                                                            Ðua na~o cho ba?n la´p va`o dâ`u ne`.\nCo´ biê´t la` lê?nh Noprefix hay không?",a.threadID);